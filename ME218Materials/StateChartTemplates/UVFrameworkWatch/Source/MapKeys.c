...
ES_Event RunMapKeys( ES_Event ThisEvent )
{
  ...
  ReturnEvent.EventType = ES_NO_EVENT; // assume no errors
    ...
    if ( ThisEvent.EventType == ES_NEW_KEY) // there was a key pressed
    {
        ...
        {
          // use keys to generate the relevant watch events
            case 'T' : ThisEvent.EventType = ES_TickEVT; 
              break;
            case 'S' : ThisEvent.EventType = ES_SetEVT;
              break;
            case 'M' : ThisEvent.EventType = ES_ModeEVT; 
              break;
            default  :
              break;
        }
     ...
     if ( ThisEvent.EventType == ES_TIMEOUT) // there was a key pressed
    {
      ThisEvent.EventType = ES_TickEVT; 
      PostMasterSM(ThisEvent);
    }
